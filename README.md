...
Параметр epsilon = 0.3 влияет на быстроту развития политики во время обучения. График растёт вверх линейно, но медленно
...
В ходе лабораторной работы интегрирована экономическая система в проект Unity и обучен ML-Agent. Выполнена работа с параметрами в .yaml файле: меняла значения для изменения обучения модели. Выяснено как некоторые параметры могут влиять на тренировку модели: learning_rate - начальная скорость обучения,beta делает политику более случайной,epsilon влияет на быстроту развития политики во время обучения, lambd используется при расчете обобщенной оценки преимущества (GAE), num_epoch количество проходов через буфер опыта при выполнении оптимизации. Для поиска наилучшего обучения можно эксперементировать с параметрами в .yaml файле и при этом анализировать графики.
